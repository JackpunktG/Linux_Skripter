...
#include <sys/syscall.h>
#define REQUEST_MEMORY(size) mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0)
#define FREE_MEMORY(ptr, size) munmap(ptr, size)
#elif defined(_WIN32)
#include <windows.h>
#define REQUEST_MEMORY(size) VirtualAlloc(NULL, size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)
#define FREE_MEMORY(ptr, size) VirtualFree(ptr, 0, MEM_RELEASE)
#else
#include <stdlib.h>
#include <string.h>
...
static Arena_Block* arena_add_block(Arena *arena, size_t minimumSize)
{
    ...
    Arena_Block* block = (Arena_Block*)REQUEST_MEMORY(sizeof(Arena_Block));
    if(!block)
    {
...
Arena* arena_init(size_t block_size, size_t alignment)
{
    ...
    Arena* arena = (Arena*)REQUEST_MEMORY(sizeof(Arena));
    if (!arena)
    {
        printf("ERROR - arena_init REQUEST_MEMORY failed\n");
        return NULL;
    }
